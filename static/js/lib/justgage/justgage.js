 ...
 * @author Bojan Djuricic (@Toorshia)
  ...
  * instead of left
  * 26 June 2019 - B. Taylor, made tol attribute optional, and renamed pki attributes
  * to distinguish between gauge path (regularpki) and level (nullpki).
**/
...
JustGage = function(config) {
  ...
  obj.config = {
    ...
    tolerance: kvLookup('tolerance', config, dataset, false),

    // fromZero : bool
    // starts pointer from zero
    fromZero: kvLookup('fromZero', config, dataset, false)
  };
  ...
  // regularpki - custom attribute for generating gauge paths
  obj.canvas.customAttributes.regularpki = function(value, min, max, w, h, dx, dy, gws, donut, reverse) {
  ...
  // nullpki - custom attribute for generating gauge paths starting from zero
  obj.canvas.customAttributes.nullpki = function(value, min, max, w, h, dx, dy, gws, donut, reverse, fromZero) {
    ...
    // var clear
    alpha, Ro, Ri, Cx, Cy, Xo, Yo, Xi, Yi, path, sweep1, sweep2 = null;
  };
  ...
  if (obj.config.counter === true) {
    ...
    eve.on("raphael.anim.frame." + (obj.level.id), function() {
      var currentValue = obj.level.attr("nullpki")[0];
      if (obj.config.reverse) {
        currentValue = (obj.config.max * 1) + (obj.config.min * 1) - (obj.level.attr("nullpki")[0] * 1);
      }
      if (obj.config.textRenderer) {
...
function getColor(val, pct, col, noGradient, custSec) {
  ...
  var noGradient = noGradient || Object.entries(custSec).length > 0;

  if (Object.entries(custSec).length > 0) {
